...
-- Potential challenges with model / information:
  -- If there are moons of moons db structure would need to change
...
(
  ...
  orbits_star_id INTEGER REFERENCES stars ON DELETE CASCADE,
...
(
  id SERIAL PRIMARY KEY,
  name TEXT NOT NULL,
  orbits_planet_id INTEGER REFERENCES planets ON DELETE CASCADE
);
